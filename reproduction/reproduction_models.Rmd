...
# indepently transform and scale the three sets
music.descriptors.train <- format.as.train(music.descriptors.train)
music.descriptors.validation <- format.as.train(music.descriptors.validation)
music.descriptors.test <- format.as.train(music.descriptors.test)
```
...
With the balanced dataset, the random forest model is performing significantly better, giving an out-of-bag accuracy of ~45%, a ~10% improvement over the imbalanced dataset, but behind the optimized CNN model using the MFCC descriptors. It is possible that tuning of the random forest hyperparameters would provide further increases in accuracy.
...
train.set.Y <- to_categorical(as.numeric(music.descriptors.train$genre))[, 2:8]
validation.set.X <- as.matrix(select(music.descriptors.validation, -genre, -song))
validation.set.Y <- to_categorical(as.numeric(music.descriptors.validation$genre))[, 2:8]

# define the model
model <- keras_model_sequential()
model %>%
  layer_dense(units = 64, activation = "relu", input_shape = 191) %>%
...
nnet.training <- fit(model, x = train.set.X, y = train.set.Y, 
                     epochs = 50, initial_epoch = 0, batch_size = 128, verbose = 1,
                     validation_data = list(validation.set.X, validation.set.Y))
```

This network reaches ~55% validation accuracy before it begins to overfit around epoch 50.

```{r}
## predict test set classes using the fitted model
# format the test set into a matrix
test.set.X <- as.matrix(select(music.descriptors.test, -genre, -song))

# make predictions on the test set
descriptors_prediction <- predict(model, test.set.X)
```
