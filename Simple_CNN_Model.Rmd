---
title: "Simple_CNN_Model"
author: "Jake Smith"
date: "8/11/2019"
---

Fit a simple convolutional neural network. Our data has three "channels" of MFCC descriptors pulled around the 25/50/75th percentile timepoints of the song. We will start by fitting a network on a single channel using the architecture employed by M. Dong (https://arxiv.org/pdf/1802.09697.pdf).
...
Using an 80/20/20 train/validation/test split.
...
data.generator <- function(data.indices, batch.size, subset = 0) {
  ...
  function() {
    ...
    # if requested, truncate X
    if (subset > 0) {
      # cut sample down to subset datapoints
      z <- sample(0:(dim(X)[2] - subset), 1)
      X <- X[, z:(z + subset), , ]
    }
...
Initial single channel architecture:
...
model %>%
  ...
  layer_lambda(function(x) x[, , , 1], input_shape = c(sample_length, 64, 3)) %>%
...
When trained with the entire 10s segment at once, this single channel model gives ~50% validation set accuracy by epoch ~10 before starting to overfit the data. There appears to be an issue with some of the MFCC descriptors in the validation set. Before scaling to the the multi-channel model, we will need to implement a filter in the data generator.
